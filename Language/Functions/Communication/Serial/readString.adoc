...
[float]
=== Beschreibung
`Serial.readString()` liest Zeichen aus dem seriellen Puffer in einen String. Die Funktion wird abgebrochen, wenn eine Zeitüberschreitung auftritt (siehe link:../settimeout[setTimeout()]).

`Serial.readString()` erbt von der link:../../stream[Stream]-Dienstklasse.
[%hardbreaks]
