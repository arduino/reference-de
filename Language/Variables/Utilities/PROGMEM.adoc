...
`String`s (die auch Arrays sind) und weiteren komplizierteren Datenstrukturen der Fall.
...
Wenn mit einer großen Menge an Text gearbeitet wird (Beispiel: Projekt mit LCD-Display), ist es häufig nötig, Texte in ein Array von `String`s zu packen.
Dadurch, dass `String`s bereits selbst Arrays sind, entsteht dadurch ein 2-dimensionales Array.
