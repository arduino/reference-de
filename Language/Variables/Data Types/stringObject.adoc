...
[float]
=== Beschreibung
Erstellt eine Instanz der `String`-Klasse. Es gibt mehrere Versionen, die `String`s aus verschiedenen Datentypen erstellen (d. h. als Zeichenfolgen formatieren), darunter:
...
* Ein konstanter int oder long int, die eine angegebene Basis verwendet
