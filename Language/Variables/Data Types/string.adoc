...
[float]
=== Syntax
Alle folgenden Angaben sind gültige Deklarationen für `String`s.
...
*Möglichkeiten zur Deklaration von `String`s*
...
Im Allgemeinen werden Zeichenfolgen mit einem Nullzeichen (ASCII-Code 0) abgeschlossen. Dadurch können Funktionen (wie `_Serial_.print()`) erkennen, wo das Ende eines `String`s ist.
...
`String`s werden immer in doppelten Anführungszeichen ("Abc") und chars in einfachen Anführungszeichen ('A') definiert.
...
Da `String`s selbst Arrays sind, handelt es sich eigentlich um ein zweidimensionales Array.
